...
export default function SidebarAdmin({ active, onLogout, user }) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-56 bg-emerald-900 text-white shadow-lg flex flex-col z-40">
      <div className="px-6 py-6 border-b border-emerald-800 flex items-center">
        <img src="/img/logo-kab-pacitan.png" alt="Logo" className="w-12 h-12 rounded-full" />
        <span className="font-bold text-lg text-center">Dashboard Desa Bandar</span>
      </div>
      <nav className="flex-1 py-6 px-4 flex flex-col gap-2">
        ...
        <Link href="/admin/struktur-organisasi" className={`flex items-center gap-3 px-4 py-2 rounded-lg font-semibold hover:bg-emerald-800 transition ${active === "struktur-organisasi" ? "bg-emerald-800" : ""}`}> <Users className="w-5 h-5" /> Struktur Organisasi </Link>
      </nav>
      <div className="px-6 py-4 border-t border-emerald-800 flex flex-col gap-1">
        ...
        >
          ...
          <span className="flex-1 truncate">{user?.name || user?.username || "Profil"}</span>
